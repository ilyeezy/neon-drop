...
// Веса сверены с замером выдачи референсной игры жанра (111 фигур, разбор
// кадров в tools/reference): у неё заметно больше линий и почти нет больших
// углов — линию легко пристроить куда угодно, а угол из пяти клеток игрок
// воспринимает как подарок, который некуда деть.
// Точка
add('P1', [[0, 0]], 6);
// Линии в двух ориентациях — самое частое семейство
const lineWeights = { 2: 12, 3: 13, 4: 10, 5: 6 };
...
add('R23', rect(2, 3), 10);
add('R32', rect(3, 2), 10);
// Малые углы: 3 клетки в габарите 2×2. Держим редкими: под ниши их и так
// подкладывает помощь, а в общей выдаче они мельчат игру.
addRotations('C3', [[0, 0], [1, 0], [0, 1]], 2, 4);
// Большие углы: 5 клеток в габарите 3×3 — самая неудобная форма в жанре
addRotations('C5', [[0, 0], [1, 0], [2, 0], [0, 1], [0, 2]], 1, 4);
// T-формы
addRotations('T4', [[0, 0], [1, 0], [2, 0], [1, 1]], 7, 4);
// S / Z — по две различимые ориентации
addRotations('S4', [[1, 0], [2, 0], [0, 1], [1, 1]], 8, 2);
addRotations('Z4', [[0, 0], [1, 0], [1, 1], [2, 1]], 8, 2);
// J / L — по четыре
addRotations('J4', [[0, 0], [0, 1], [1, 1], [2, 1]], 5, 4);
addRotations('L4', [[2, 0], [0, 1], [1, 1], [2, 1]], 5, 4);
